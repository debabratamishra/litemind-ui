...
docs/                   extended documentation, design records, and Astro site
...
Is it a new LLM provider?
  → Implement in backend/app/services/llm_gateway.py; add a case to resolve_backend_config()

Is it a new type of chat capability?
  → Add a Skill in backend/app/skills/ implementing StreamingChatSkill protocol
  → Register it in ChatSkillRegistry (backend/app/skills/registry.py)
...
Is it a new document format?
  → Extend backend/app/ingestion/file_ingest.py (format detection + extraction)

Is it a realtime-voice capability?
  → WebRTC signaling: backend/app/backend/api/voice.py (POST /api/voice/offer)
  → Pipecat pipeline: backend/app/services/voice_pipeline.py (build_voice_pipeline / run_voice_pipeline)
  → Voice is a SEPARATE pipeline, not a Skill — never add it to backend/app/skills/
  → Event contract: docs/superpowers/specs/2026-07-18-realtime-voice-mode-design.md

Is it a new API endpoint?
  → Add route handler in backend/app/backend/api/
  → Add Pydantic request/response models in backend/app/backend/models/
...
- Tests live in `tests/`. Mirror the `backend/app/` structure (e.g. `tests/services/test_rag_service.py`).
...
- Sanitise all file names and paths using `backend/app/backend/api/security_utils.py`
