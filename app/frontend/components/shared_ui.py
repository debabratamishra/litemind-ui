...
OPENROUTER_DEFAULT_API_BASE = (
    os.getenv("OPENROUTER_API_BASE") or os.getenv("OPENROUTER_BASE_URL") or "https://openrouter.ai/api/v1"
)
OPENROUTER_DEFAULT_MODEL = os.getenv("DEFAULT_OPENROUTER_MODEL", "openai/gpt-4o-mini")
NVIDIA_NIM_DEFAULT_API_BASE = os.getenv("NVIDIA_NIM_API_BASE") or "https://integrate.api.nvidia.com/v1"
NVIDIA_NIM_DEFAULT_MODEL = os.getenv("DEFAULT_NVIDIA_NIM_MODEL", "meta/llama3-70b-instruct")

HOSTED_BACKEND_CONFIGS: Dict[str, Dict[str, str]] = {
    "openrouter": {
        "display_name": "OpenRouter",
        "api_base_key": "openrouter_api_base",
        "api_key_key": "openrouter_api_key",
        "default_api_base": OPENROUTER_DEFAULT_API_BASE,
        "default_model": OPENROUTER_DEFAULT_MODEL,
        "model_prefix": "openrouter/",
        "model_example": "meta-llama/llama-3.3-70b-instruct or openai/gpt-4o-mini",
        "embedding_model_example": "openai/text-embedding-3-small",
        "api_key_env_var": "OPENROUTER_API_KEY",
    },
    "nvidia_nim": {
        "display_name": "Nvidia NIM",
        "api_base_key": "nvidia_nim_api_base",
        "api_key_key": "nvidia_nim_api_key",
        "default_api_base": NVIDIA_NIM_DEFAULT_API_BASE,
        "default_model": NVIDIA_NIM_DEFAULT_MODEL,
        "model_prefix": "nvidia_nim/",
        "model_example": "meta/llama3-70b-instruct or mistralai/mixtral-8x7b-instruct",
        "embedding_model_example": "nvidia/nv-embedqa-e5-v5",
...
def render_generation_settings(prefix: str, expanded: bool = True) -> GenerationConfig:
    """
...
def render_memory_config(
    memory_manager: Any, prefix: str, history_key: str = "history_enabled", memory_key: str = "memory_enabled"
) -> None:
    """
...
def render_backend_selector() -> str:
    ...
    if backend in HOSTED_BACKEND_CONFIGS:
        provider_config = get_hosted_backend_config(backend)
        api_base_key = provider_config["api_base_key"]
        api_key_key = provider_config["api_key_key"]
        display_name = provider_config["display_name"]
        model_prefix = provider_config["model_prefix"]

        st.session_state.setdefault(api_key_key, "")
        st.session_state.setdefault(api_base_key, provider_config["default_api_base"])
        st.sidebar.caption(
            f"{display_name} model ids can be entered as provider/model. The app prefixes {model_prefix} automatically."
        )
        st.sidebar.text_input(
            f"{display_name} API Base",
            key=api_base_key,
...
def get_hosted_backend_config(backend_provider: str) -> Dict[str, str]:
    """Return UI metadata for a hosted LiteLLM backend."""
    try:
        return HOSTED_BACKEND_CONFIGS[backend_provider]
    except KeyError as exc:
        raise ValueError(f"Unsupported hosted backend: {backend_provider}") from exc


def is_hosted_backend(backend_provider: str) -> bool:
    """Return True when the backend uses remote LiteLLM credentials."""
    return backend_provider in HOSTED_BACKEND_CONFIGS


def get_default_model_for_backend(backend_provider: str) -> str:
    """Return the default model shown in the UI for the given backend."""
    if backend_provider == "ollama":
        return "default"
    return get_hosted_backend_config(backend_provider)["default_model"]
...
def get_backend_request_config(backend_provider: str) -> Dict[str, Optional[str]]:
    ...
    if backend_provider in HOSTED_BACKEND_CONFIGS:
        provider_config = get_hosted_backend_config(backend_provider)
        api_base = (st.session_state.get(provider_config["api_base_key"]) or "").strip()
        api_key = (st.session_state.get(provider_config["api_key_key"]) or "").strip()
...
def create_simple_summary(messages: List[Dict[str, str]], existing_summary: Optional[str] = None) -> str:
    """
