...
from .enhanced_document_processor import (  # noqa: E402
...
class EnhancedImageProcessor:
    ...
    def _extract_ocr_text(self, img_array: np.ndarray, metadata: dict) -> Optional[dict]:
        ...
        try:
            ...
            for variant in self._build_ocr_variants(img_array):
                try:
                    ocr_results.extend(ocr_reader.readtext(variant))
    ...
    def _analyze_image_properties(self, image: Image.Image, metadata: dict) -> Optional[dict]:
...
class EnhancedCSVProcessor:
    ...
    def _extract_large_csv_streaming(self, path: Path) -> List[dict]:
        ...
        try:
            ...
            blocks[0]["content"] = str(blocks[0]["content"]) + f"\nActual rows processed: {total_rows}"
    ...
    def _analyze_column(self, df: pd.DataFrame, column: str, filename: str) -> Optional[dict]:
