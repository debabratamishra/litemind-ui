...
class EnhancedDocumentProcessor:
    ...
    def _extract_page_text_enhanced(self, page, page_num: int, filename: str) -> List[dict]:
        ...
        try:
            ...
            self._analyze_text_layout(text_dict)
    ...
    def _run_page_ocr(self, image: Image.Image) -> Tuple[str, int]:
        ...
        for variant in variants:
            try:
                results = ocr_reader.readtext(variant)
    ...
    def _analyze_document_structure(self, path: Path) -> Optional[dict]:
        ...
        try:
            ...
            content = f"""DOCUMENT STRUCTURE ANALYSIS: {path.name}
                ...
                Document Metadata:
                • Title: {doc_metadata.get('title', 'Not specified')}
                • Author: {doc_metadata.get('author', 'Not specified')}
                • Creator: {doc_metadata.get('creator', 'Not specified')}
                • Subject: {doc_metadata.get('subject', 'Not specified')}
